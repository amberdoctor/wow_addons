...
function Race:update()
    ...
    for i=1, artifactCount do
        local name, description, _, icon, _, keystoneCount, _, completionTime, completionCount = GetArtifactInfoByRace(self.id, i)
        local artifact = Artifact:getByName(name)
        if artifact then
            artifact:update()
...
function Race:getActiveArtifact()
    ...
    return Artifact:getByName(name)
end

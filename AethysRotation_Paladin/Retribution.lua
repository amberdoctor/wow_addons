  ...
  -- In Combat
    ...
    if Target:Exists() and Player:CanAttack(Target) and not Target:IsDeadOrGhost() then
      ...
      -- actions+=/call_action_list,name=priority
        ...
        local Var_Crusade = (not S.Crusade:IsAvailable() or (S.Crusade:CooldownRemains() > Player:GCD() * 3)) and AR.CDsON() or (not AR.CDsON() or Settings.Retribution.OffGCDasOffGCD.Crusade[1]);
        if Judged() then
